...
# system.time(
# optim(c(2.14,1.34), fn=.marg, method="L-BFGS-B",
#         lower=c(eps, eps), 
#         upper=c(Inf, Inf), cons=f[[i]], 
#         y1=sample[[i]], y2=control[[i]], ncomp=ncomp)$par)
# system.time(
# nlminb(start=c(2.14,1.34), objective=.marg, cons=f[[i]], 
...
# system.time(solnp(c(2.14,1.34), fun=.marg, 
#         eqfun=NULL, eqB=NULL, LB=c(eps, eps), 
#         UB=c(Inf, Inf), cons=f[[i]], 
...
# 
#     y <- .mydmarginalDBD(x, y1=y1[u], y2=y2[u], al=al[u], 
#         bl=bl[u], W=W[u], control.available=control.available, 
